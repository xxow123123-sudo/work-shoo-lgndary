...
import { currentSession, sessionAvatarUrl } from './auth';
...
export async function currentAccess(){
  ...
  if(!session) return {user:null,discordId:'',role:null as StaffRole|null,employee:null,locked:false,avatarUrl:null,username:''};
  const displayName=session.displayName||session.username;
  const user={id:session.discordId,user_metadata:{full_name:displayName,name:displayName,preferred_username:session.username,avatar_hash:session.avatar||null}};
  ...
  if(employee && (!employee.is_active || employee.employment_status!=='active')) return {user,discordId,role:null as StaffRole|null,employee,locked:true,avatarUrl,username:session.username};
  ...
  if(!live) return {user,discordId,role:null as StaffRole|null,employee,locked:false,avatarUrl,username:session.username};
  let record=employee;
  if(!record && live!=='employee'){
    const {data}=await db.from('employees').upsert({discord_user_id:discordId,discord_username:displayName,role:live,is_active:true,employment_status:'active'},{onConflict:'discord_user_id'}).select('*').single();
    record=data;
  ...
  return {user,discordId,role:live,employee:record,locked:false,avatarUrl,username:session.username};
}
