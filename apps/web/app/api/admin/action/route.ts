...
export async function POST(req:Request){
  try{
    ...
    if(body.action==='set_mod_requirement'){
      ...
      await audit(actor,'weekly_mod_requirement','guild',process.env.DISCORD_GUILD_ID!,{count}); return NextResponse.json({ok:true});
    }
    if(body.action==='resource_delivery'){
      ...
      await audit(actor,'weekly_resources_received','employee',emp.id,{quantity,cycle_id:cycle.id}); return NextResponse.json({ok:true});
    }
