...
export async function POST(req:Request){
  try{
    ...
    if(body.action==='recruitment'){
      ...
      await audit(actor,body.open?'recruitment_open':'recruitment_close','guild',process.env.DISCORD_GUILD_ID!); return NextResponse.json({ok:true});
    }
    if(body.action==='terminate'){
      ...
      await sendDiscord('admin_logs_channel_id',{content:`⛔ **فصل موظف**\nالموظف: <@${body.discord_id}>\nبواسطة: <@${actor}>\nالسبب: ${body.reason||'-'}`}); await audit(actor,'employee_terminated','employee',emp.id,{reason:body.reason}); return NextResponse.json({ok:true});
    }
    if(body.action==='reactivate'){
      ...
      await updateDiscordMemberRole(body.discord_id,process.env.DISCORD_EMPLOYEE_ROLE_ID||process.env.DISCORD_EMPLOYEES_ROLE_ID,true); await audit(actor,'employee_reactivated','employee',emp.id,{}); return NextResponse.json({ok:true});
    }
    if(body.action==='close_week'){
      ...
      if(rows?.length) await db.from('weekly_snapshots').upsert(rows.map((r:any)=>({cycle_id:cycle.id,employee_id:r.employee_id,tool_sales:r.tool_sales||0,vehicle_mods:r.vehicle_mods||0,points:r.points||0,invoice_total:r.invoice_total||0})),{onConflict:'cycle_id,employee_id'});
      const now=new Date().toISOString(); await db.from('weekly_cycles').update({is_current:false,ends_at:now,closed_by_discord_id:actor}).eq('id',cycle.id); await db.from('weekly_cycles').insert({starts_at:now,is_current:true}); await audit(actor,'close_week','weekly_cycle',cycle.id,{snapshot_count:rows?.length||0}); return NextResponse.json({ok:true});
    }
